...
import {
  // Pastikan ini memanggil API retur, BUKAN pembelian biasa.
...
export default function MenuDetailReturNotaPembelianBasetroli() {
  ...
  const id_toko = parseInt(
    useSelector((state) => state.auth.id_toko),
    10
  );
  ...
  // 6) STATE FILTER
  // ganti filter "Kode Retur" -> filter "Kode Pembelian"
  ...
  // -------------------------------
  // FUNGSI BANTUAN (FORMAT RUPIAH & TANGGAL)
  // -------------------------------
  ...
  useEffect(() => {
    const fetchHeaderRetur = async () => {
      try {
        ...
        // Filter dengan id_retur_pembelian + id_toko
        const selectedHeader = headerArray.find(
          (item) =>
            item.id_retur_pembelian?.toString() === id_retur_pembelian &&
            parseInt(item.id_toko, 10) === id_toko
        );
    ...
    fetchHeaderRetur();
  }, [id_retur_pembelian, id_toko, token]);
  ...
  useEffect(() => {
    const fetchDetailRetur = async () => {
      try {
        ...
        // Filter by id_retur_pembelian dan id_toko
  ...
  useEffect(() => {
    ...
    // Filter Kode Pembelian
    if (filterKodePembelian) {
      temp = temp.filter((item) =>
        item.kode_pembelian
          ?.toLowerCase()
          .includes(filterKodePembelian.toLowerCase())
      );
    }
  ...
  // -------------------------------
  // 7. JIKA LOADING ATAU ERROR
  // -------------------------------
  if (loading) return <Loading />;
  if (error) return <Error message={error} />;

  // -------------------------------
  // 8. BUAT OPSI UNIK UNTUK FILTERSELECT
  // -------------------------------
  // Ganti uniqueKodeRetur -> uniqueKodePembelian
  ...
  return (
    <div className="py-14" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="bg-white rounded-md shadow-md p-2 mb-2 border border-gray-200">
        ...
        {/* Jika headerData == null, berarti tidak ditemukan */}
        {headerData ? (
      ...
      <div className="bg-white flex flex-col md:flex-row rounded-md shadow-md p-2 mb-2 border border-gray-200 gap-4">
        {/* 
          Ubah Label: "Filter Kode Retur" -> "Filter Kode Pembelian" 
          Agar menyesuaikan filterKodePembelian
        */}
        <FilterSelect
          ...
          <table className="w-full text-xs text-left text-gray-500 border-collapse border">
            <thead className="text-xs text-blue-900 uppercase bg-gray-200 sticky top-0">
              <tr>
                ...
                <th
                  className="px-1 py-0.5 w-28 border border-gray-500 cursor-pointer"
                ...
                <th
                  className="px-1 py-0.5 w-28 border border-gray-500 cursor-pointer"
                  onClick={() => handleSort("kode_pembelian")}
                >
                  Kode Pembelian
                  {sortConfig.key === "kode_pembelian" &&
                    (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                <th
                  className="px-1 py-0.5 w-20 border border-gray-500 cursor-pointer"
                ...
                <th
                  className="px-1 py-0.5 w-52 border border-gray-500 cursor-pointer"
                ...
                <th
                  className="px-1 py-0.5 w-20 border border-gray-500 cursor-pointer"
                  onClick={() => handleSort("qtyBeli")}
                >
                  Qty Order
                  {sortConfig.key === "qtyBeli" &&
                    (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                <th
                  className="px-1 py-0.5 w-20 border border-gray-500 cursor-pointer"
                  onClick={() => handleSort("qtyReturBeli")}
                >
                  Qty Retur
                  {sortConfig.key === "qtyReturBeli" &&
                    (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                <th
                  className="px-1 py-0.5 w-28 border border-gray-500 cursor-pointer"
                ...
                <th
                  className="px-1 py-0.5 w-32 border border-gray-500 cursor-pointer"
              ...
              {currentItems.length > 0 ? (
                currentItems.map((item, index) => (
                  <tr key={index} className="text-gray-900">
                    ...
                    {/* 
                      Jika di detail TIDAK ada "kode_retur_pembelian", 
                      fallback ke headerData.kode_retur_pembelian atau "-" 
                    */}
                    <td className="px-1 py-0.5 border border-gray-500">
          ...
          <div className="flex items-center space-x-2">
            <span className="text-xs text-gray-500">
              ...
              <span className="font-semibold text-gray-900">
                {indexOfFirstItem + 1} -{" "}
                {Math.min(indexOfLastItem, sortedDetail.length)}
              </span>{" "}
              of{" "}
              <span className="font-semibold text-gray-900">
                {sortedDetail.length}
