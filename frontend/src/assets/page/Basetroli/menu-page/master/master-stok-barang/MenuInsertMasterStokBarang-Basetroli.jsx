...
export default function MenuInsertMasterStokBarangBasetroli() {
  ...
  // Ubah id_toko ke number (jika diperlukan)
  ...
  // Ambil data untuk dropdown nama toko
  useEffect(() => {
    const fetchToko = async () => {
      try {
        const tokoData = await getLaporanMasterTokoInsertDataBarang(token);
        // Jika data berupa objek, konversi ke array
        const tokoArray = Array.isArray(tokoData)
          ? tokoData
          : Object.values(tokoData);
        // Filter hanya toko dengan id_toko yang sama dengan authToko
        const filteredToko = tokoArray.filter(
          (item) => parseInt(item.id_toko, 10) === authToko
        );
  ...
  // Ambil data untuk dropdown kode barang
  useEffect(() => {
    const fetchBarang = async () => {
      try {
        ...
        // Filter hanya barang yang memiliki id_toko yang sama dengan authToko
        const filteredBarang = barangArray.filter(
          (item) => parseInt(item.id_toko, 10) === authToko
        );
        // Mapping opsi dengan format "kode_barang - namabarang"
        const options = filteredBarang.map(
    ...
    <div className="py-14 mb-10" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="head flex justify-between items-center">
        <div className="breadcrumb flex items-center">
          ...
          <Link to="/dashboard/basetroli/menu/masterstokbarang/insert" className="text-xs font-semibold text-gray-400">
              Tambah Stok
          </Link>
        </div>
        <button
