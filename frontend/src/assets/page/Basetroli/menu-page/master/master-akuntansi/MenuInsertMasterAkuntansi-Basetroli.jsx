...
export default function MenuinsertMasterAkuntansiMaster() {
  ...
  useEffect(() => {
    const fetchMasterAkun = async () => {
      try {
        ...
        // Kita tidak langsung set masterAkunOptions, karena akun akan difilter berdasarkan toko yang dipilih.
      } catch (err) {
  ...
  useEffect(() => {
    const fetchMasterToko = async () => {
      try {
        ...
        const dataRows = Array.isArray(data) ? data : Object.values(data);
        // Map data ke dalam bentuk options
        const options = dataRows.map((item) => ({
          value: item.id_toko,
          label: item.nama_toko,
        }));
        // Filter options hanya menampilkan toko yang id_toko-nya sama dengan id_toko dari state
        const filteredOptions = options.filter(
          (option) => option.value === id_toko
        );
        setMasterTokoOptions(filteredOptions);
      } catch (err) {
  ...
  useEffect(() => {
    if (!formData.id_toko) {
      // Jika belum memilih toko, tampilkan kosong untuk dropdown akun
      setMasterAkunOptions([]);
      setFormData((prev) => ({ ...prev, id_master_akun: "" }));
    } else {
      // Filter allMasterAkun berdasarkan toko yang dipilih
      ...
      // Reset nilai akun jika sebelumnya sudah terpilih dan tidak sesuai dengan toko baru
      setFormData((prev) => ({ ...prev, id_master_akun: "" }));
    }
