...
import NavbarUp from "../../Bigtroli/navbar/NavbarUp-Bigtroli";
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        const masterLokasiMap = new Map(
          masterLokasiArr.map((l) => [l.id_lokasi, l.nama_lokasi])
        );

        // Gabungkan id_lokasi dari seluruh sumber
        const lokasiDariPenjualan = rows
          .map((r) => r.id_lokasi)
          .filter(Boolean);
        const lokasiDariProgress = orderArr
          .map((o) => o.id_store)
          .filter(Boolean);
                    ...
                    <div className="mt-2 m-5 text-sm">
                      <div className="flex">
                        ...
                        <div className="flex w-40 items-center gap-2">
                          <div className="">
                            <img src={LogoBasket} className="w-7 h-7" alt="" />
                          </div>{" "}
