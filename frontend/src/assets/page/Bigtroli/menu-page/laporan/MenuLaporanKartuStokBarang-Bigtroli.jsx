...
import { getLaporanKartuStokBarangAll } from "../../../../services/apiService";
...
export default function MenuLaporanKartuStokBarangRetur() {
    ...
    const fetchData = async () => {
      try {
        ...
        const filteredDataRows = dataRows.filter((row) => {
          return parseInt(row.id_toko, 10) === idTokoInt;
        });
  ...
  useEffect(() => {
    ...
    // Filter tambahan dengan searchTerm jika diperlukan
    if (searchTerm) {
    ...
    setCurrentPage(1);
  }, [kodeBarang, namaBarang, startDate, endDate, noTransaksi, kodePackinglist, searchTerm, data]);
              ...
              {currentItems.length > 0 ? (
                currentItems.map((item, index) => (
                  <tr key={`${item.kode_barang}-${index}`} className="border-b text-gray-700">
                    ...
                    <td className="px-1 py-0.5 border border-gray-700">
                      {item.kode_packinglist || "Data tidak ditemukan"}
                    </td>
                    <td className="px-1 py-0.5 border border-gray-700">
