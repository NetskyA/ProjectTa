...
function addNotaFooterAt(doc, width, startY, height, subTotal, totalNoPesanan, catatan, terbilang) {
  doc.setFontSize(7).setFont("courier", "normal");
  doc.text(`Sub Total           : ${formatRp(subTotal)}`, 10, startY);
  doc.text(`Total No. Pemesanan : ${totalNoPesanan}`, 10, startY + 4);
  doc.text(`Terbilang           : ${terbilang(subTotal)} Rupiah`, 10, startY + 8);

  const noteY = startY + 12;
  doc.rect(10, noteY, width - 20, 15);
  doc.setFontSize(8).text("Catatan:", 12, noteY + 4);
  doc.text(catatan || "-", 28, noteY + 4);

  const sigY = height - 20;
  const gap = 19;
  const usableWidth = width - 20;
  const colW = usableWidth / 3;
  const x1 = 25, x2 = x1 + colW, x3 = x1 + 2 * colW;

  doc.setFontSize(9);
  doc.text("Penerima", x1 + 12, sigY);
  doc.text("(_____________________)", x1, sigY + gap);
  doc.text("(_____________________)", x2 + colW / 2 - 25, sigY + gap);
  doc.text("Hormat Kami, DOUBLE O BAKERY", x3 - 35, sigY);
  doc.text("(_____________________)", x3, sigY + gap);
}

function addNotaFooter(doc, width, height, subTotal, totalNoPesanan, catatan, terbilang) {
  const startY = 20;
  addNotaFooterAt(doc, width, startY, height, subTotal, totalNoPesanan, catatan, terbilang);
}
    ...
    <div className="py-14" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="head flex justify-between items-center">
        <div className="flex items-center">
          ...
          <Link
            to="/dashboard/adminkitchen/menu/notapenjualan/adminkitchen"
            className="text-xs font-semibold text-blue-900"
          >
            Nota Penjualan
          </Link>
