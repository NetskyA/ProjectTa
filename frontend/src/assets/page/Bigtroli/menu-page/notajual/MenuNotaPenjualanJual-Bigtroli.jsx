...
import { getMasterNotaPenjualan } from "../../../../services/apiService";
...
export default function MenuNotaPembelianBasetroli() {
  ...
  const handleSort = (key) =>
    ...
    (async () => {
      try {
        setLoading(true);
        const res = await getMasterNotaPenjualan(token);
        // console.log("nota penjualan", res);
        let rows = [];
...
const formatDateNoTime = (isoString) => {
  ...
  useEffect(() => {
    ...
    // only filter by kode_master_nota_penjualan and createat date
    if (kodeProduksiFilter) {
      f = f.filter(r => r.kode_master_nota_penjualan === kodeProduksiFilter);
    }
    ...
    if (searchTerm) {
      ...
      f = f.filter(r =>
        r.kode_master_nota_penjualan.toLowerCase().includes(q) ||
  ...
  const detailGabunganPermintaan = (id) => {
    // console.log("Navigating to detail with ID:", id);
    navigate(`/dashboard/adminkitchen/menu/notapenjualan/adminkitchen/detail/${id}`);
  };
    ...
    <div className="py-14" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="bg-white flex flex-col md:flex-row rounded-md shadow-md p-2 justify-between items-center border border-gray-200 mb-1">
        ...
        <FilterSelect
            ...
            options={[...new Set(data.map(r => r.kode_master_nota_penjualan))]}
                ...
                pageRows.map((r, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Kode GP */}
                      <td className="px-2 py-0.5 border border-gray-500 uppercase">
                      {r.kode_master_nota_penjualan}
                      </td>

                      {/* Kitchen */}
                      <td className="px-2 py-0.5 border border-gray-500 uppercase">
                      {formatDateNoTime(r.createat)}
                      </td>
