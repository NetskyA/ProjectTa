    ...
    setTableRows((prevRows) =>
      prevRows.map((row, i) => {
        if (i === index) {
          ...
          if (newQty > parseInt(row.quantity, 10)) {
            setAlert({
              message: `Qty Retur tidak boleh lebih dari ${row.quantity}.`,
              type: "warning",
              visible: true,
            });
            setTimeout(() => {
  ...
  const handleExportPDF = () => {
    try {
      ...
      const tableBody = tableRows.map((row) => [
        ...
        row.total_harga !== undefined ? row.total_harga : "-",
        row.nama_user || "-",
        row.createAt || "-",
      ]);
      doc.autoTable({
  ...
  const handleConfirmRetur = async () => {
    try {
      ...
      if (response.success) {
        ...
        setTimeout(() => {
          navigate("/dashboard/bigtorlly/menu/notapenjualan/returpenjualan");
        }, 2000);
      } else {
        setAlert({
    ...
    <div className="py-14" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="head flex justify-between items-center">
        <div className="cover flex items-center">
          ...
          <Link
            to="/dashboard/bigtorlly/menu/notapenjualan/returpenjualan"
            className="text-xs font-semibold text-blue-900"
          >
            Retur Penjualan
          </Link>
          ...
          <table className="w-full text-xs text-left text-gray-500 border-collapse">
            <thead className="text-xs text-blue-900 uppercase bg-gray-200 sticky top-0">
              <tr>
                ...
                <th className="px-1 py-0.5 border border-gray-500">
                  No Pemesanan
                </th>
                <th className="px-1 py-0.5 border border-gray-500">
                  Kode Pemesanan
                ...
                <th className="px-1 py-0.5 border border-gray-500">Besar</th>
                <th className="px-1 py-0.5 border border-gray-500">Sedang</th>
                <th className="px-1 py-0.5 border border-gray-500">Kecil</th>
                <th className="px-1 py-0.5 border border-gray-500">
                  Quantity
                </th>
                <th className="px-1 py-0.5 border border-gray-500">
              ...
              {tableRows.length > 0 ? (
                tableRows.map((row, index) => (
                  <tr key={`${row.no_pemesanan}-${index}`} className="text-gray-900">
                    ...
                    {/* Kolom Qty Retur dengan input per baris */}
                    <td className="px-1 py-0.5 border border-gray-500">
                      <input
                        type="text"
                        placeholder="0"
                    ...
                    {/* Kolom Action untuk menghapus baris */}
                    <td className="px-1 py-0.5 border border-gray-500">
