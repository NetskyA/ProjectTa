...
export default function MenuReturNotaPenjualanBigroli() {
  ...
  const detailNotaJual = (idRetur) => {
    ...
    navigate(`/dashboard/bigtorlly/menu/notapenjualan/returpenjualan/proses/${idRetur}`);
  };
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-2 space-y-2 md:space-y-0">
        ...
        <div className="flex justify-end">
          <button
            onClick={() =>
              navigate(
                "/dashboard/bigtorlly/menu/notapenjualan/returpenjualan/add"
              )
            }
            className="toadd w-14 h-7 text-xs rounded-md border-2 text-gray-100 bg-blue-900 hover:bg-blue-600 transition duration-300"
          >
            Tambah
          </button>
        </div>
            ...
            <tbody>
              {currentItems.length > 0 ? (
                currentItems.map((item, index) => {
                  ...
                  return (
                    <tr
                      key={`${item.kode_pemesanan}-${item.kode_barang}-${index}`}
                      className={`text-gray-900 ${bgColor}`}
                    >
                ...
                <tr>
                  <td colSpan={17} className="px-4 py-1 text-center text-gray-500">
