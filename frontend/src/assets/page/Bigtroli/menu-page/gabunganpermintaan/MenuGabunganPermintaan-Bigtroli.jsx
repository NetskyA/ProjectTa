// MenuNotaPembelianBasetroli.jsx

import { useState, useEffect, useMemo, useRef } from "react";
import { Link, useNavigate } from "react-router-dom";
import { getMasterGabunganPermintaan } from "../../../../services/apiService";
...
export default function MenuNotaPembelianBasetroli() {
  ...
  const [kodeGabungan, setKodeGabungan] = useState("");
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const res = await getMasterGabunganPermintaan(token);
        let rows = [];
  ...
  useEffect(() => {
    let f = data;
    if (kodeGabungan)
      f = f.filter((r) => r.kode_gabungan_permintaan === kodeGabungan);
    if (kodePesanan)
    ...
    if (tanggalBuatFilter) {
      f = f.filter(
        (r) => r.createat && r.createat.slice(0, 10) === tanggalBuatFilter
      );
    }
    ...
    if (searchTerm) {
      ...
      f = f.filter(
        (r) =>
          ...
          (r.kode_gabungan_permintaan || "").toLowerCase().includes(q) ||
  ...
  const detailGabunganPermintaan = (id) => {
    console.log("Navigating to detail with ID:", id);
    navigate(`/dashboard/adminkitchen/menu/gabunganpermintaan/detail/${id}`);
  };
    ...
    <div className="py-14" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="bg-white flex flex-col md:flex-row rounded-md shadow-md p-2 justify-between items-center border border-gray-200 mb-1">
        <div className="flex text-xs flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 w-full">
          <FilterSelect
            label="No. Gabungan"
            options={uniqueKodeGP}
            value={kodeGabungan}
            onChange={setKodeGabungan}
          />
          <FilterSelect
            label="Kitchen"
            options={uniqueKitchenName}
            value={namaKitchen}
            onChange={setNamaKitchen}
          />
          <div className="flex flex-col text-xs">
            <label className="mb-1 text-blue-900 font-semibold">Tanggal</label>
            <input
              type="date"
      ...
      <div className="bg-white rounded-md shadow-md border border-gray-200 p-1.5 mb-2">
        <div className="flex items-center justify-center mb-2 relative">
          ...
          <div className="w-fit flex absolute right-1 items-center justify-center bg-gray-200 rounded-md p-0.5 cursor-pointer">
            <Link to="/dashboard/adminkitchen/menu/gabunganpermintaan/add">
            ...
            <thead className="text-xs font-semibold text-blue-900 uppercase bg-gray-200">
              <tr>
                ...
                <th
                  ...
                  onClick={() => handleSort("tanggal_verifikasi_gb")}
                >
                  Tgl Verifikasi GB
                  {sortConfig.key === "tanggal_verifikasi_gb" &&
                    (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                ...
                <th
                  scope="col"
                  className="px-2 py-0.5 w-40 cursor-pointer sticky top-0 border border-gray-500 bg-gray-200 z-10"
                  onClick={() => handleSort("status_gabungan")}
                >
                  Status Gabungan
                  {sortConfig.key === "status_gabungan" &&
                    (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                <th
                  scope="col"
                  className="px-2 py-0.5 w-40 cursor-pointer sticky top-0 border border-gray-500 bg-gray-200 z-10"
                  onClick={() => handleSort("total_gabungan")}
                >
                  Total Gabungan
                  {sortConfig.key === "total_gabungan" &&
                    (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                ...
                pageRows.map((r, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Tgl Verifikasi GB */}
                      <td className="px-2 py-0.5 border border-gray-500">
                        {r.tanggal_verifikasi_gb
                          ? r.tanggal_verifikasi_gb.slice(0, 10)
                          : "-"}
                      </td>
                        ...
                        <button
                          onClick={() =>
                            detailGabunganPermintaan(
                              r.id_master_gabungan_pemintaan
                            )
                          }
                        >
                          {/* ikon “eye” */}
