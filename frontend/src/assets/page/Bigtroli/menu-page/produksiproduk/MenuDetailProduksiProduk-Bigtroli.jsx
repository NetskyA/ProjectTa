...
------------------------------------------------------------------------ */
  ...
  const handleVerifyPengeluaran = async () => {
    if (!produksiHeader?.id_master_produksi) {
      return setAlert({
        message: "Data produksi belum lengkap.",
        type: "warning",
        visible: true,
      });
    }
  
    // 1. Siapkan array details (unique per PB + SO)
    const detailsRaw = tableRows.map((r) => ({
      id_master_pesanan_pembelian: r.pbId,
      id_master_sales_order:      r.soId,
    }));
    const details = Array.from(
      new Map(
        detailsRaw.map((d) => [
          `${d.id_master_pesanan_pembelian}-${d.id_master_sales_order}`,
          d,
        ])
      ).values()
    );
  
    // 2. Siapkan array bahanBakuList
    const bahanBakuList = bahanBakuAggregated.map((b) => ({
      id_bahan_baku:   b.id_bahan_baku,
      jumlah_dipotong: b.totalKebutuhan,       // ambil dari kolom Total Kebutuhan (gr)
    }));
  
    // 3. Bentuk payload lengkap
    const payload = {
      id_master_produksi:      produksiHeader.id_master_produksi,
      tanggal_verifikasi_pr:   nowSQL(),       // "YYYY-MM-DD HH:mm:ss"
      createby:                Number(id_user),
      details,                                  // [{…}, …]
      bahanBakuList,                            // [{…}, …]
    };
    ...
    try {
      setLoading(true);
      const res = await updatePrTanggalVerifikasi(token, payload);
      ...
      if (res.success) {
        ...
        setProduksiHeader((h) => ({
          ...h,
          status_produksi:       1,
          tanggal_verifikasi_pr: payload.tanggal_verifikasi_pr,
        }));
    ...
    <div className="py-14" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="head flex justify-between items-center">
        <div className="flex items-center">
          ...
          <Link
            to="/dashboard/adminkitchen/menu/produksiproduk"
            className="text-xs font-semibold text-blue-900"
          >
            Detail
          </Link>
          ...
          <div className="flex-row text-xs">
            <div className="flex gap-1.5">
              ...
              <div className="w-40">
                ...
                <input
                  disabled
                  value={produksiHeader?.kode_master_produksi || "-"}
                  className="w-40 border h-7 text-gray-500 border-gray-300 rounded p-1 bg-gray-100"
                />
              </div>
              <div className="w-40">
                ...
                <input
                  ...
                  value={
                    produksiHeader?.tanggal_verifikasi_pr
                      ? formatDate(produksiHeader.tanggal_verifikasi_pr)
                      : "-"
                  }
                  className="w-full border h-7 text-gray-500 border-gray-300 rounded p-1 bg-gray-100"
                />
            ...
            <div className="flex gap-1.5 mt-2">
              ...
              <div className="w-40">
                ...
                <input
                  disabled
                  value={
                    produksiHeader?.status_bukti_pengeluran === 0
                      ? "MENUNGGU VER. PENGELUARAN"
                      : "Lanjut Bukti Pengeluaran"
                  }
                  className="w-full border h-7 uppercase text-gray-500 border-gray-300 rounded p-1 bg-gray-100"
                />
              </div>
        ...
        <fieldset className="flex flex-col h-full space-y-0.5">
          <fieldset className="border h-full border-blue-500 rounded-md p-2 shadow-sm">
            ...
            <button
              ...
              className="flex items-center space-x-2 border w-48 bg-gray-100 border-gray-300 rounded p-1 text-blue-900 hover:bg-gray-100"
            >
              <img src={LogoPrint} alt="print" className="w-6 h-6" />
              <span className="text-sm font-semibold">Cetak Produksi</span>
            </button>
          </fieldset>
          <fieldset className="border border-blue-500 h-full rounded-md p-2 shadow-sm">
            <legend className="px-2 text-sm font-bold text-blue-900">
              Verifikasi B. Pengeluaran
            </legend>
            <button
              disabled={isPRVerified}
              onClick={isPRVerified ? undefined : handleVerifyPengeluaran}
