...
useEffect(() => {
  const fetchBahanBaku = async () => {
    try {
      ...
      const found = dataArray.find(item => item.id_bahan_baku === parseInt(id_bahan_baku));
      if (found) {
...
const handleSubmit = async (e) => {
  ...
  try {
    if (id_bahan_baku) {
      // 🔄 Mode Update
const payload = {
  id_bahan_baku: parseInt(id_bahan_baku),  // 🔍 Pastikan ini bernilai
  id_kategori_bahan_baku: parseInt(id_kategori_bahan_baku),
  kode_bahan_baku: selectedData?.kode_bahan_baku || "",
  nama_bahan_baku: nama_satuan,
  id_satuan: parseInt(id_satuan),
  id_lokasi: parseInt(id_lokasi),
};

      const result = await updateMasterBahanBaku(token, payload);
