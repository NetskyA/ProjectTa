/// MenuInsertMasterAkun-Master.jsx
...
export default function MenuInsertMasterAkunMaster() {
  ...
  const [masterTokoList, setMasterTokoList] = useState([]); // State untuk menampung daftar toko

  const navigate = useNavigate();
  const dispatch = useDispatch();

  // Ambil token, id_user, id_toko dari Redux
  ...
  useEffect(() => {
    const fetchMasterToko = async () => {
      try {
        ...
        // Tampilkan semua data toko (tanpa filter):
        setMasterTokoList(dataToko);

        // -- Jika Anda ingin mem-filter berdasarkan id_toko, gunakan baris di bawah -- 
        // const filtered = dataToko.filter((t) => t.id_toko === id_toko);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setTimeout(() => {
          setAlert({ message: "", type: "", visible: false });
          navigate("/dashboard/master/menu/akun");
        }, 2000);
      } else {
        setAlert({
    ...
    <div className="py-14 mb-10" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="head flex justify-between items-center">
        <div className="breadcrumb flex items-center">
          ...
          <Link to="/dashboard/master/menu/akun" className="text-xs font-semibold text-blue-900">
            Master Akun
          </Link>
          <div className="mx-1">
            <svg
