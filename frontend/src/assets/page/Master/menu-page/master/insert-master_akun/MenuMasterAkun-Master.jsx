...
import { getMasterAkun, deleteMasterAkun } from "../../../../../services/apiService";
...
export default function MenuMasterAkunMaster() {
  ...
  // Ambil data master akun tanpa filter id_toko
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Hapus filter id_toko sehingga semua data ditampilkan
        setData(dataRows);
      ...
      filtered = filtered.filter((item) => {
        return (
          ...
          String(item.created_at || "")
            .toLowerCase()
            .includes(lower) ||
          String(item.status || "").toLowerCase().includes(lower)
        );
      });
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setFilteredData(filteredData.filter((item) => item.id_master_akun !== selectedAkunId));
    } catch (err) {
  ...
  // Edit handler
  const handleEdit = (id) => {
    navigate(`/dashboard/master/menu/akun/update/${id}`);
  };
    ...
    <div className="py-14 mb-10" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="head flex justify-between items-center">
        <div className="breadcrumb flex items-center">
          ...
          <Link to="/dashboard/master/menu/akun" className="text-xs font-semibold text-gray-400">
            Master Akun
          </Link>
        </div>
        <button
      ...
      {/* ========== Table Section ========== */}
      <div className="bg-white rounded-md shadow-md border border-gray-200 p-1.5 mt-2 overflow-x-auto" style={{ maxHeight: "60vh" }}>
      <div className="flex items-center justify-center mb-2 relative">
          ...
          <div className="absolute right-0 flex space-x-2">
            <Link to="/dashboard/master/menu/akun/insert">
        ...
        <table className="w-full text-xs text-left text-gray-500 border">
          <thead className="text-xs text-blue-900 uppercase font-semibold bg-gray-200">
            <tr>
              <th className="px-1 py-0.5 w-10 cursor-pointer sticky top-0 border border-gray-700 bg-gray-200" onClick={() => handleSort("id_master_akun")}>
                ID
                {sortConfig.key === "id_master_akun" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
              </th>
              <th className="px-1 py-0.5 w-32 cursor-pointer sticky top-0 border border-gray-700 bg-gray-200" onClick={() => handleSort("nama_akun")}>
                Nama Akun
                {sortConfig.key === "nama_akun" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
              </th>
              <th className="px-1 py-0.5 w-28 cursor-pointer sticky top-0 border border-gray-700 bg-gray-200" onClick={() => handleSort("kode_akun")}>
                Kode Akun
                {sortConfig.key === "kode_akun" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
              </th>
              <th className="px-1 py-0.5 w-28 cursor-pointer sticky top-0 border border-gray-700 bg-gray-200" onClick={() => handleSort("nama_toko")}>
                Nama Toko
                {sortConfig.key === "nama_toko" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
              </th>
              <th className="px-1 py-0.5 w-32 cursor-pointer sticky top-0 border border-gray-700 bg-gray-200" onClick={() => handleSort("created_by_user")}>
                DI BUAT OLEH
                {sortConfig.key === "created_by_user" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
              </th>
              <th className="px-1 py-0.5 w-36 cursor-pointer sticky top-0 border border-gray-700 bg-gray-200" onClick={() => handleSort("created_at")}>
                DI BUAT TANGGAL
                {sortConfig.key === "created_at" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
              </th>
              <th className="px-1 py-0.5 w-20 cursor-pointer sticky top-0 border border-gray-700 bg-gray-200" onClick={() => handleSort("status")}>
                Status
          ...
          <tbody>
            {currentItems.length > 0 ? (
              currentItems.map((item, index) => {
                ...
                return (
                  <tr key={`${item.id_master_akun}-${index}`} className={`text-gray-700 border-b ${rowBg}`}>
                    <td className="px-1 py-0.5 border border-gray-700">{item.id_master_akun}</td>
                    <td className="px-1 py-0.5 border border-gray-700">{item.nama_akun || "-"}</td>
                    <td className="px-1 py-0.5 border border-gray-700">{item.kode_akun || "-"}</td>
                    ...
                    <td className="px-1 py-0.5 border border-gray-700">{formatDate(item.created_at) ? formatDate(item.created_at): "-"}</td>
                    <td className="px-1 py-0.5 border border-gray-700">
                      {item.status === 0 ? "Aktif" : "Non-Aktif"}
                    </td>
                    <td className="px-1 py-0.5 border border-gray-700">
                      <div className="flex justify-center space-x-2">
                        <button onClick={() => handleEdit(item.id_master_akun)} title="Edit">
                        ...
                        <button onClick={() => handleDeleteClick(item.id_master_akun)} title="Delete">
              ...
              <tr>
                <td colSpan="7" className="px-1 py-0.5 text-center">Tidak ada data yang tersedia</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        <ul className="inline-flex items-stretch -space-x-px">
          <li>
            <button
              ...
              className={`flex items-center justify-center py-1 px-1 ml-0 text-gray-700 bg-white rounded-l-md text-xs border border-gray-300 hover:bg-gray-100 hover:text-gray-700 ${currentPage === 1 ? "cursor-not-allowed opacity-50" : ""}`}
            >
              Previous
            </button>
          </li>
          <li>
            <button
              ...
              className={`flex items-center justify-center text-xs py-1 px-1 text-gray-700 bg-white rounded-r-md border border-gray-300 hover:bg-gray-100 hover:text-gray-700 ${currentPage === totalPages ? "cursor-not-allowed opacity-50" : ""}`}
            >
              Next
            </button>
          </li>
