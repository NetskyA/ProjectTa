...
export default function MenuUpdateMasterAkun() {
  ...
  const [masterTokoList, setMasterTokoList] = useState([]); // State untuk menyimpan data toko
    ...
    const fetchData = async () => {
      try {
        ...
        setFormData({
          ...
          id_toko: selected.id_toko || "", // pastikan field id_toko ada pada data
  ...
  // Ambil data master toko untuk dropdown
  useEffect(() => {
    const fetchToko = async () => {
      try {
        const tokoData = await getMasterToko(token);
        setMasterTokoList(tokoData);
      } catch (error) {
