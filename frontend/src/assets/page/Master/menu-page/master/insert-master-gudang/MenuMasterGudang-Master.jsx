...
import { getMasterGudang, deleteMasterGudang } from "../../../../../services/apiService";
...
export default function MenuLaporanMasterGudang() {
  ...
  // FILTERS
  const [kodeGudangFilter, setKodeGudangFilter] = useState("");
  const [namaGudangFilter, setNamaGudangFilter] = useState("");
  // Untuk status, kita gunakan "Semua", "Aktif", "Non-Aktif"
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log("Data Master Gudang (original):", result);

        const formattedData = Array.isArray(result) ? result : Object.values(result);
        setData(formattedData);
        setFilteredData(formattedData);
      } catch (err) {
        console.error("Error fetching master gudang data:", err);
        setError(err.message || "Failed to fetch master gudang data.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const uniqueKodeGudang = [...new Set(data.map((item) => item.kode_gudang || ""))];
  const uniqueNamaGudang = [...new Set(data.map((item) => item.nama_gudang || ""))];
  // Status options untuk FilterSelect
  ...
  useEffect(() => {
    ...
    if (kodeGudangFilter) {
      filtered = filtered.filter((item) => item.kode_gudang === kodeGudangFilter);
    }

    if (namaGudangFilter) {
      filtered = filtered.filter((item) => item.nama_gudang === namaGudangFilter);
    }
    ...
    if (searchTerm) {
      filtered = filtered.filter((item) => {
        const { id_gudang, kode_gudang, nama_gudang, alamat, created_by_user, createAt } = item;
    ...
    setCurrentPage(1);
  }, [searchTerm, kodeGudangFilter, namaGudangFilter, statusFilter, data]);
  ...
  // Handler untuk Edit: navigasi ke halaman update dengan mempassing id_gudang
  const handleEdit = (id_gudang) => {
    console.log("Navigating to update page with id_gudang:", id_gudang);
    navigate(`/dashboard/master/menu/master/gudang/update/${id_gudang}`);
  };

  // Handler untuk menampilkan dialog delete
  const handleDeleteClick = (id_gudang) => {
    setDeleteId(id_gudang);
    setIsDialogOpen(true);
  };

  // Konfirmasi delete: memanggil deleteMasterGudang dan mengupdate state
  const confirmDelete = async () => {
    try {
      await deleteMasterGudang(deleteId, token);
      setAlert({ message: "Data berhasil dinonaktifkan.", type: "success", visible: true });
      const updatedData = data.filter((item) => item.id_gudang !== deleteId);
      setData(updatedData);
    ...
    <div className="py-14" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="head flex justify-between items-center mb-0.5">
        <div className="cover flex items-center">
          ...
          <span className="text-xs font-semibold text-gray-400">Master Gudang</span>
        </div>
      ...
      <div className="bg-white flex flex-col md:flex-row flex-wrap rounded-md shadow-md p-4 justify-between items-center border border-gray-200 mb-2">
        <div className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 w-full">
          <FilterSelect
            label="Filter Kode Gudang"
            options={uniqueKodeGudang.map(String)}
            value={kodeGudangFilter}
            onChange={setKodeGudangFilter}
          />
          <FilterSelect
            label="Filter Nama Gudang"
            options={uniqueNamaGudang.map(String)}
            value={namaGudangFilter}
            onChange={setNamaGudangFilter}
          />
          <FilterSelect
            label="Filter Status"
      ...
      <div className="bg-white rounded-md shadow-md border border-gray-200 p-1.5 max-w-4xl">
        <div className="flex items-center justify-center mb-2 relative">
          <p className="text-xl font-semibold text-blue-900 absolute left-1">Master Gudang</p>
          <div className="w-2/6">
          ...
          <div className="absolute right-0 flex space-x-2">
            <Link to="/dashboard/master/menu/master/gudang/insert">
          ...
          <table className="w-full text-xs text-left text-gray-500 border-collapse border">
            <thead className="text-xs font-semibold text-blue-900 uppercase bg-gray-200 w-full">
              <tr>
                ...
                >
                  Kode Gudang
                  {sortConfig.key === "kode_gudang" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                <th
                ...
                >
                  Nama Gudang
                  {sortConfig.key === "nama_gudang" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                <th
                ...
                >
                  Alamat
                  {sortConfig.key === "alamat" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                <th
                ...
                <th
                  scope="col"
                  className="px-1 py-0.5 sticky top-0 border border-gray-700 bg-gray-200 text-blue-900 z-10 cursor-pointer"
                >
                  Action
                </th>
              </tr>
                ...
                currentItems.map((item, index) => {
                  ...
                  return (
                    <tr key={index} className={`border-b text-gray-900 ${rowBg}`}>
                      ...
                      <td className="px-1 py-0.5 border border-gray-700">{item.kode_gudang}</td>
                      <td className="px-1 py-0.5 border border-gray-700">{item.nama_gudang}</td>
                      <td className="px-1 py-0.5 border border-gray-700">{item.alamat || "-"}</td>
                      ...
                      <td className="px-1 py-0.5 border border-gray-700">
                        <div className="flex justify-center space-x-2">
                          <button onClick={() => handleEdit(item.id_gudang)} title="Edit">
                            <svg
                          ...
                          <button onClick={() => handleDeleteClick(item.id_gudang)} title="Delete">
                            <svg
        ...
        <DialogTrueFalse
          ...
          message="Apakah Anda yakin ingin menonaktifkan data gudang ini?"
