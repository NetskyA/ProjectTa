...
import { getLaporanMasterRole, deleteMasterRole } from "../../../../../services/apiService";
...
export default function MenuLaporanMasterRole() {
  ...
  // FILTERS
  const [kodeRoleFilter, setKodeRoleFilter] = useState("");
  const [namaRoleFilter, setNamaRoleFilter] = useState("");
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        setError(null);
        const result = await getLaporanMasterRole(token);
        const formattedData = Array.isArray(result) ? result : Object.values(result);
        const filteredResult = formattedData.filter(item => parseInt(item.id_role, 10) !== 3);
        setData(filteredResult);
        setFilteredData(filteredResult);
      } catch (err) {
        console.error("Error fetching master role data:", err);
        setError(err.message || "Failed to fetch master role data.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const uniqueKodeRole = [...new Set(data.map((item) => item.kode_role || ""))];
  const uniqueNamaRole = [...new Set(data.map((item) => item.nama_role || ""))];
  ...
  useEffect(() => {
    ...
    if (kodeRoleFilter) {
      filtered = filtered.filter((item) => item.kode_role === kodeRoleFilter);
    }
    if (namaRoleFilter) {
      filtered = filtered.filter((item) => item.nama_role === namaRoleFilter);
    }
    ...
    if (searchTerm) {
      filtered = filtered.filter((item) => {
        const { id_role, kode_role, nama_role, createBy, created_by_user, createAt } = item;
    ...
    setCurrentPage(1);
  }, [searchTerm, kodeRoleFilter, namaRoleFilter, statusFilter, data]);
  ...
  const handleEdit = (id_role) => {
    console.log("Navigating to update page with id_role:", id_role);
    navigate(`/dashboard/master/menu/master/role/update/${id_role}`);
  };

  // Handler untuk Delete
  const handleDeleteClick = (id_role) => {
    setDeleteId(id_role);
  ...
  const confirmDelete = async () => {
    try {
      await deleteMasterRole(deleteId, token);
      setAlert({ message: "Data berhasil dinonaktifkan.", type: "success", visible: true });
      const updatedData = data.filter((item) => item.id_role !== deleteId);
      setData(updatedData);
    ...
    <div className="py-14" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="head flex justify-between items-center mb-0.5">
        <div className="cover flex items-center">
          ...
          <span className="text-xs font-semibold text-gray-400">Master Akses</span>
        </div>
      ...
      <div className="bg-white flex flex-col md:flex-row flex-wrap rounded-md shadow-md p-4 justify-between items-center border border-gray-200 mb-2">
        <div className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 w-full">
          <FilterSelect
            label="Filter Kode Role"
            options={uniqueKodeRole.map(String)}
            value={kodeRoleFilter}
            onChange={setKodeRoleFilter}
          />
          <FilterSelect
            label="Filter Nama Role"
            options={uniqueNamaRole.map(String)}
            value={namaRoleFilter}
            onChange={setNamaRoleFilter}
          />
          <FilterSelect
            label="Filter Status"
      ...
      <div className="bg-white rounded-md shadow-md border border-gray-200 p-1.5 max-w-4xl">
        <div className="flex items-center justify-center mb-2 relative">
          <p className="text-xl font-semibold text-blue-900 absolute left-1">Master Akses</p>
          <div className="w-2/6">
          ...
          <div className="absolute right-0 flex space-x-2">
            <Link to="/dashboard/master/menu/master/role/insert">
          ...
          <table className="w-full text-xs text-left text-gray-500 border-collapse border">
            <thead className="text-xs font-semibold text-blue-900 uppercase bg-gray-200 w-full">
              <tr>
                ...
                >
                  Nama Role {sortConfig.key === "nama_role" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                <th
                  onClick={() => handleSort("kode_role")}
                  className="px-1 py-0.5 w-36 sticky top-0 border border-gray-700 bg-gray-200 text-blue-900 z-10 cursor-pointer"
                >
                  Kode Role {sortConfig.key === "kode_role" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                <th
                ...
                currentItems.map((item, index) => {
                  const rowBg =
                    item.status === 0
                      ...
                      : "bg-white";
                  return (
                    <tr key={index} onDoubleClick={() => handleEdit(item.id_role)} className={`border-b cursor-pointer hover:opacity-75 text-gray-900 ${rowBg}`}>
                      <td className="px-1 py-0.5 border border-gray-700">{indexOfFirstItem + index + 1}</td>
                      <td className="px-1 py-0.5 border border-gray-700">{item.nama_role}</td>
                      <td className="px-1 py-0.5 border border-gray-700">{item.kode_role}</td>
                      ...
                      <td className="px-1 py-0.5 border border-gray-700">
                        <div className="flex justify-center space-x-2">
                          <button onClick={() => handleEdit(item.id_role)} title="Edit">
                          ...
                          <button onClick={() => handleDeleteClick(item.id_role)} title="Delete">
        ...
        <DialogTrueFalse
          ...
          message="Apakah Anda yakin ingin menonaktifkan data role ini?"
