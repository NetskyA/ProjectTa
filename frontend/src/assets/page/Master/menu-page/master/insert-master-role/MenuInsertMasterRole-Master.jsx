...
import { insertMasterRole } from "../../../../../services/apiService"; // Pastikan fungsi API ini benar
...
export default function MenuInsertMasterRole() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validasi: field nama_role harus terisi
    if (!nama_role) {
      setAlert({
    ...
    // Siapkan payload, kode_role dibiarkan kosong karena backend yang menangani generate
    const payload = {
      kode_role: "", // akan di-generate oleh backend (angka increment)
      nama_role,
    ...
    setLoading(true);
    try {
      const result = await insertMasterRole(payload, token);
      setAlert({
      ...
      console.error("Error inserting master role:", err);
      setAlert({
    ...
    <div className="py-14 mb-10" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="head flex justify-between items-center">
        <div className="breadcrumb flex items-center">
          ...
          <Link to="/dashboard/master/menu/master/role/insert" className="text-xs font-semibold text-gray-400">
            Tambah Akses
          </Link>
        </div>
        <button
      ...
      <div className="bg-white flex rounded-md shadow-md p-4 justify-between items-center border border-gray-200 mb-1">
        <p className="text-xl font-semibold text-blue-900">Master Akses</p>
      ...
      {/* Form Insert Master Role */}
      <div className="p-2 max-w-2xl mt-2 bg-white rounded-md shadow-md border border-gray-300">
        <h2 className="text-lg font-semibold mb-2">Detail Akses</h2>
        <form onSubmit={handleSubmit}>
          <div className="mb-4 flex gap-4">
            {/* Input Kode Role - dibiarkan kosong dan auto-generate oleh backend */}
