...
import { getLaporanMasterDepo, deleteMasterDepo } from "../../../../../services/apiService";
...
export default function MenuLaporanMasterDepo() {
  ...
  // Untuk status, kita gunakan nilai "Semua", "Aktif", "Non-Aktif"
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Error fetching master depo data:", err);
        setError(err.message || "Failed to fetch master depo data.");
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter((item) => {
        const { id_depo, kode_depo, nama_depo, alamat_depo, createBy, created_by_user, createAt } = item;
    ...
    setCurrentPage(1);
  }, [searchTerm, kodeDepoFilter, namaDepoFilter, statusFilter, data]);
  ...
  // Handler untuk Edit: navigasi ke halaman update dengan mempassing id_depo
  const handleEdit = (id_depo) => {
    console.log("Navigating to update page with id_depo:", id_depo);
    navigate(`/dashboard/master/menu/master/depo/update/${id_depo}`);
  };

  // Handler untuk menampilkan dialog delete
  const handleDeleteClick = (id_depo) => {
    setDeleteId(id_depo);
    setIsDialogOpen(true);
  };

  // Konfirmasi delete: memanggil deleteMasterDepo dan mengupdate state
  const confirmDelete = async () => {
    try {
      await deleteMasterDepo(deleteId, token);
      setAlert({ message: "Data berhasil dinonaktifkan.", type: "success", visible: true });
      // Update state data dengan menghapus data yang telah didelete
      const updatedData = data.filter((item) => item.id_depo !== deleteId);
      setData(updatedData);
    ...
    <div className="py-14" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="bg-white flex flex-col md:flex-row flex-wrap rounded-md shadow-md p-4 justify-between items-center border border-gray-200 mb-2">
        <div className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 w-full">
          <FilterSelect
            ...
            options={uniqueNamaDepo.map(String)}
            value={namaDepoFilter}
            onChange={setNamaDepoFilter}
          />
          <FilterSelect
            label="Filter Kode Supplier"
            options={uniqueKodeDepo.map(String)}
            value={kodeDepoFilter}
            onChange={setKodeDepoFilter}
          />
          <FilterSelect
            label="Filter Status"
      ...
      <div className="bg-white rounded-md shadow-md border border-gray-200 p-1.5 max-w-4xl">
        <div className="flex items-center justify-center mb-2 relative">
          <p className="text-xl font-semibold text-blue-900 absolute left-1">Master Supplier</p>
          <div className="w-2/6">
          ...
          <div className="absolute right-0 flex space-x-2">
            <Link to="/dashboard/master/menu/master/depo/insert">
          ...
          <table className="w-full text-xs text-left text-gray-500 border-collapse border">
            <thead className="text-xs font-semibold text-blue-900 uppercase bg-gray-200 w-full">
              <tr>
                ...
                >
                  Kode Supplier
                  {sortConfig.key === "kode_depo" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                <th
                ...
                >
                  Alamat Supplier
                  {sortConfig.key === "alamat_depo" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                <th
                ...
                currentItems.map((item, index) => {
                  ...
                  return (
                    <tr key={index} className={`border-b text-gray-900 ${rowBg}`}>
                      ...
                      <td className="px-1 py-0.5 border border-gray-700">{item.kode_depo}</td>
                      <td className="px-1 py-0.5 border border-gray-700">{item.nama_depo}</td>
                      <td className="px-1 py-0.5 border border-gray-700">
                        {item.alamat_depo || "-"}
                      </td>
                      <td className="px-1 py-0.5 border border-gray-700">
                      ...
                      <td className="px-1 py-0.5 border border-gray-700">
                        <div className="flex justify-center space-x-2">
                          <button onClick={() => handleEdit(item.id_depo)} title="Edit">
                            <svg
                          ...
                          <button onClick={() => handleDeleteClick(item.id_depo)} title="Delete">
                            <svg
        ...
        <DialogTrueFalse
          ...
          message="Apakah Anda yakin ingin menonaktifkan data depo ini?"
