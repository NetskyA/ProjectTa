...
import { getLaporanMasterMarketPlace, deleteMasterMarketPlace } from "../../../../../services/apiService";
import { useSelector } from "react-redux";
import Loading from "../../../../component/Loading";
import Error from "../../../../component/Error";
import Alert from "../../../../component/Alert";
import DialogTrueFalse from "../../../../component/DialogTrueFalse"; // Pastikan path-nya sesuai
...
export default function MenuLaporanMasterMarketplace() {
  ...
  const [statusFilter, setStatusFilter] = useState(""); // "" (semua), "0" (Aktif), "1" (Non-Aktif)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Ganti API dengan getLaporanMasterMarketPlace
        const result = await getLaporanMasterMarketPlace(token);
        console.log("Data Master Marketplace (original):", result);
  ...
  const uniqueKodeMarketplace = [...new Set(data.map((item) => item.kode_marketplace || ""))];
  const uniqueNamaMarketplace = [...new Set(data.map((item) => item.nama_marketplace || ""))];
  const statusOptions = ["", "0", "1"]; // "" untuk semua, "0" untuk Aktif, "1" untuk Non-Aktif
  ...
  useEffect(() => {
    ...
    if (kodeMarketplaceFilter) {
      filtered = filtered.filter((item) => item.kode_marketplace === kodeMarketplaceFilter);
    }

    if (namaMarketplaceFilter) {
      filtered = filtered.filter((item) => item.nama_marketplace === namaMarketplaceFilter);
    }
    ...
    if (searchTerm) {
      filtered = filtered.filter((item) => {
        const { id_marketplace, kode_marketplace, nama_marketplace, alamat, createBy, created_by_user, createAt } = item;
    ...
    setCurrentPage(1);
  }, [searchTerm, kodeMarketplaceFilter, namaMarketplaceFilter, statusFilter, data]);
  ...
  // Handler untuk Edit: navigasi ke halaman update dengan mempassing id_marketplace
  const handleEdit = (id_marketplace) => {
    console.log("Navigating to update page with id_marketplace:", id_marketplace);
    navigate(`/dashboard/master/menu/master/marketplace/update/${id_marketplace}`);
  };

  // Handler untuk menampilkan dialog delete
  const handleDeleteClick = (id_marketplace) => {
    setDeleteId(id_marketplace);
    setIsDialogOpen(true);
  };

  // Konfirmasi delete: memanggil deleteMasterMarketPlace dan mengupdate state
  const confirmDelete = async () => {
    try {
      await deleteMasterMarketPlace(deleteId, token);
      setAlert({ message: "Data berhasil dinonaktifkan.", type: "success", visible: true });
      const updatedData = data.filter((item) => item.id_marketplace !== deleteId);
      setData(updatedData);
    ...
    <div className="py-14" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="head flex justify-between items-center mb-0.5">
        <div className="cover flex items-center">
          ...
          <span className="text-xs font-semibold text-gray-400">Master Marketplace</span>
        </div>
      ...
      <div className="bg-white flex flex-col md:flex-row flex-wrap rounded-md shadow-md p-4 justify-between items-center border border-gray-200 mb-2">
        <div className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 w-full">
          <FilterSelect
            label="Filter Kode Marketplace"
            options={uniqueKodeMarketplace.map(String)}
            value={kodeMarketplaceFilter}
            onChange={setKodeMarketplaceFilter}
          />
          <FilterSelect
            label="Filter Nama Marketplace"
            options={uniqueNamaMarketplace.map(String)}
      ...
      <div className="bg-white rounded-md shadow-md border border-gray-200 p-1.5 max-w-4xl">
        <div className="flex items-center justify-center mb-2 relative">
          <p className="text-xl font-semibold text-blue-900 absolute left-1">Master Marketplace</p>
          <div className="w-2/6">
          ...
          <div className="absolute right-0 flex space-x-2">
            <Link to="/dashboard/master/menu/master/marketplace/insert">
          ...
          <table className="w-full text-xs text-left text-gray-500 border-collapse border">
            <thead className="text-xs font-semibold text-blue-900 uppercase bg-gray-200 w-full">
              <tr>
                ...
                <th
                  onClick={() => handleSort("kode_marketplace")}
                  className="px-1 py-0.5 w-36 sticky top-0 border border-gray-700 bg-gray-200 text-blue-900 z-10 cursor-pointer"
                >
                  Kode Marketplace {sortConfig.key === "kode_marketplace" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                <th
                ...
                >
                  Nama Marketplace {sortConfig.key === "nama_marketplace" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                <th
                  onClick={() => handleSort("alamat")}
                  className="px-1 py-0.5 w-44 sticky top-0 border border-gray-700 bg-gray-200 text-blue-900 z-10 cursor-pointer"
                >
                  Alamat {sortConfig.key === "alamat" && (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                <th
                ...
                currentItems.map((item, index) => {
                  ...
                  return (
                    <tr key={index} className={`border-b text-gray-900 ${rowBg}`}>
                      ...
                      <td className="px-1 py-0.5 border border-gray-700">{item.kode_marketplace}</td>
                      ...
                      <td className="px-1 py-0.5 border border-gray-700">
                        <div className="flex justify-center space-x-2">
                          <button onClick={() => handleEdit(item.id_marketplace)} title="Edit">
                            <svg
                          ...
                          <button onClick={() => handleDeleteClick(item.id_marketplace)} title="Delete">
                            <svg
        ...
        <DialogTrueFalse
          ...
          message="Apakah Anda yakin ingin menonaktifkan data marketplace ini?"
