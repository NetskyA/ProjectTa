...
export default function MenuNotaDetailPenjualanBesttroli() {
  ...
  // Variabel id_toko tidak digunakan lagi untuk filter
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Karena sekarang untuk master, semua data admin ditampilkan (tanpa filter id_toko)
        setData(dataRows);
  ...
  // ----------------------------------------------------------------
  //   GET UNIQUE STORES (untuk filter dropdown)
  // ----------------------------------------------------------------
  const uniqueStores = useMemo(() => {
    const stores = new Set(data.map((item) => item.nama_toko).filter(Boolean));
      ...
      if (kode) {
        if (!aggregation[kode]) {
          aggregation[kode] = {
            ...item,
            total_quantity: 0,
            // Modifikasi: total_harga dihitung sebagai harga_jual * total_quantity untuk setiap item
            total_harga: 0,
            besar: 0,
            sedang: 0,
            kecil: 0,
  ...
  // ----------------------------------------------------------------
  //   HANDLE EXPORT PDF (UBAH BAGIAN INI SAJA)
  // ----------------------------------------------------------------
  const handleExportPDF = async () => {
    try {
      // Filter data: Hanya data dengan status = 0 (belum dicetak)
      let dataToProcess = data.filter((item) => item.status === 0);
      // Jika ada filter berdasarkan nama_toko, proses hanya data dari toko tersebut
      if (storeFilter) {
        dataToProcess = dataToProcess.filter(
          (item) =>
            item.nama_toko &&
            item.nama_toko.toLowerCase().includes(storeFilter.toLowerCase())
        );
      }
      ...
      if (dataToProcess.length === 0) {
        setAlert({
          message:
            "Hanya mencetak NJ yang belum dicetak atau data tidak ditemukan sesuai filter toko.",
          type: "warning",
          visible: true,
        });
        return;
      ...
      const columnStyles = {
        0: { cellWidth: 11 },  // Kode
        1: { cellWidth: 47 },  // Nama
        2: { cellWidth: 9, halign: "right" }, // Sat
      ...
      chunks.forEach((chunk, index) => {
        ...
        autoTable(doc, {
          ...
          body: chunk.map((item) => [
            ...
            item.besar || "-",
            item.sedang || "-",
            // <--- Di sini kita pakai total_quantity,
            //      bukan lagi item.kecil
            item.total_quantity || "-",
          ]),
          startY: currentY,
  ...
  // ----------------------------------------------------------------
  //   SEARCH & FILTER (STORE) & SORTING
  // ----------------------------------------------------------------
  useEffect(() => {
    const lowercasedSearch = searchTerm.toLowerCase();
    const filtered = data.filter((item) => {
      // Filter berdasarkan nama_toko jika ada
      ...
      const matchesSearch =
        kodeBarang.toLowerCase().includes(lowercasedSearch) ||
        namaMarketplace.toLowerCase().includes(lowercasedSearch) ||
        kodePemesanan.toLowerCase().includes(lowercasedSearch) ||
        namaBarang.toLowerCase().includes(lowercasedSearch);
      return matchesStore && matchesSearch;
    });
    setFilteredData(filtered);
    setCurrentPage(1);
  }, [searchTerm, data, storeFilter]);
  ...
  return (
    <div className="py-14 font-sans" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="bg-white flex rounded-md shadow-md p-4 justify-between items-center border border-gray-200 mb-1">
        ...
        <div className="flex space-x-2">
          <button
            ...
            className="h-8 w-8 rounded-md flex items-center justify-center text-gray-100 bg-blue-900 hover:bg-blue-600 transition duration-300"
          >
      ...
      <div className="bg-white rounded-md shadow-md border border-gray-200 p-1.5">
        {/* Filter Nama Toko */}
        <div className="flex flex-col md:flex-row items-center gap-8 mb-2 space-y-4 md:space-y-0">
          <div className="flex items-center space-x-2">
            <label htmlFor="storeFilter" className="text-xs text-gray-700">
              Filter Toko:
            </label>
            <select
              id="storeFilter"
              ...
              className="border h-8 border-gray-300 rounded-md text-xs p-1"
            >
            ...
            <tbody>
              {currentItems.length > 0 ? (
                currentItems.map((group, index) => {
                  // Modifikasi: totalHarga dihitung sebagai harga_jual * total_quantity untuk setiap item di group
                    ...
                    >
                      ...
                      {expandedRows.includes(group.kode_pemesanan) && (
                        <tr>
                          <td
                            colSpan={3}
                            className="pl-4 pb-0 border border-gray-500 bg-gray-50"
                          >
