...
export default function MenuNotaDetailPenjualanBesttroli() {
  ...
  // ========== States untuk pagination, sorting, dsb. ==========
    ...
    <div className="py-14 font-sans" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="head flex justify-between items-center">
        ...
        <button
          ...
          className="w-14 h-6 text-xs rounded-md border-2 font-xs text-gray-100 bg-blue-900 hover:bg-blue-520 transition duration-300"
        >
          Refresh
        </button>
      </div>
          ...
          <table className="w-full text-xs text-left text-gray-500 border">
            <thead className="text-xs text-blue-900 uppercase bg-gray-200">
              <tr>
                <th
                  scope="col"
                  className="px-4 py-0.5 w-5 cursor-pointer sticky top-0 border border-gray-500 bg-gray-200 z-10"
                ...
                <th
                  scope="col"
                  className="px-4 py-0.5 w-72 cursor-pointer sticky top-0 border border-gray-500 bg-gray-200 z-10"
                ...
                <th
                  scope="col"
                  className="px-4 py-0.5 w-36 sticky top-0 border border-gray-500 bg-gray-200 z-10"
                >
                  Status
                </th>
                <th
                  scope="col"
                  className="px-4 py-0.5 w-36 cursor-pointer sticky top-0 border border-gray-500 bg-gray-200 z-10"
            ...
            <tbody>
              {currentItems.length > 0 ? (
                currentItems.map((group, index) => {
                  // Tentukan background berdasarkan status_packinglist (misalnya)
                  const statusPL = group.details[0].status_packinglist;
                  const backgroundClass =
                    statusPL === 0
                      ...
                      ? "bg-lime-500"
                      : "";
                  // Hitung total harga dari seluruh detail dalam group
                  const totalHarga = group.details.reduce(
                    (acc, item) =>
                      acc +
                      (parseFloat(item.harga_jual) *
                  ...
                  return (
                    ...
                    >
                      {/* Baris Utama (Header Group) */}
                      <tr
                        className={`cursor-pointer text-gray-800 ${backgroundClass}`}
                        onDoubleClick={() =>
                      ...
                      >
                        ...
                        <td className="px-4 py-0.5 border border-gray-500">
                          {displayValue(
                            group.kode_packinglist,
                            "belum dicetak"
                          )}
                        </td>
                        {/* Kolom Status */}
                        <td className="px-4 py-0.5 border border-gray-500">
                          {group.details[0].status_packinglist === 1
                            ? "Tercetak"
                            : "Belum tercetak"}
                        </td>
                        <td className="px-4 py-0.5 border border-gray-500">
                      ...
                      {/* Baris Detail (Sub Table) */}
                      {expandedRows.includes(group.kode_packinglist) && (
                        <tr>
                          <td
                            colSpan={4}
                            className="pl-4 border border-gray-500"
                          >
                            <div className="overflow-y-auto max-h-96">
                              {(() => {
                                // Pengelompokan (Aggregation) berdasarkan kombinasi kode_pemesanan dan kode_barang
                                const aggregatedDetails = Object.values(
                                  group.details.reduce((acc, item) => {
                                    const key = `${item.kode_pemesanan}_${item.kode_barang}`;
                                    if (acc[key]) {
                                      acc[key].quantity += Number(
                                        item.quantity || 0
                                      );
                                      acc[key].total_harga += Number(
                                        item.total_harga || 0
                                      );
                                      return acc;
                                    }
                                ...
                                return (
                                  <table className="w-full text-xs text-left text-gray-900">
                                    <thead
                                      className={`text-xs text-blue-900 uppercase ${backgroundClass}`}
                                    >
                                      <tr>
                                        <th className="px-1 py-0.5 w-52 border border-gray-500 sticky top-0 z-10">
                                          Nota Jual
                                        </th>
                                        <th className="px-1 py-0.5 w-52 border border-gray-500 sticky top-0 z-10">
                                          Marketplace
                                        ...
                                        <th className="px-1 py-0.5 w-80 border border-gray-500 sticky top-0 z-10">
                                          Nama Barang
                                        </th>
                                        <th className="px-1 py-0.5 w-52 border border-gray-500 sticky top-0 z-10">
                                          Quantity (pcs)
                                    ...
                                    <tbody>
                                      {aggregatedDetails.map((itemAgg, idx) => (
                                        <tr
                                          key={`${itemAgg.id_header_pemesanan}-${itemAgg.kode_barang}-${idx}`}
                                          className={backgroundClass}
                                        >
                                          <td
                                            className={`px-1 py-0.5 border border-gray-500 ${backgroundClass}`}
                                          >
                                            {itemAgg.kode_pemesanan ||
                                              "Data tidak ditemukan"}
                                          </td>
                                          <td
                                            className={`px-1 py-0.5 border border-gray-500 ${backgroundClass}`}
                                          >
                                            {itemAgg.nama_marketplace ||
                                              "Data tidak ditemukan"}
                                          </td>
                                          <td
                                            className={`px-1 py-0.5 border border-gray-500 ${backgroundClass}`}
                                          ...
                                          <td
                                            className={`px-1 py-0.5 border border-gray-500 ${backgroundClass}`}
                                          >
                                            {formatRupiah(itemAgg.harga_jual) ||
                                              "Data tidak ditemukan"}
                                          </td>
                                          <td
                                            className={`px-1 py-0.5 border border-gray-500 ${backgroundClass}`}
                ...
                <tr>
                  <td
                    colSpan={4}
                    className="px-4 py-0.5 text-center text-gray-500"
                  >
          ...
          <ul className="inline-flex items-stretch -space-x-px">
            <li>
              <button
                ...
                className={`flex items-center justify-center py-0.5 px-1 ml-0 text-gray-500 bg-white rounded-l-md text-xs border border-gray-500 hover:bg-gray-100 hover:text-gray-700 ${
            ...
            <li>
              <button
                ...
                className={`flex items-center justify-center text-xs py-0.5 px-1 text-gray-500 bg-white rounded-r-md border border-gray-500 hover:bg-gray-100 hover:text-gray-700 ${
