...
export default function MenuReturNotaPenjualanBigroli() {
  ...
  const [resultData, setResultData] = useState([]); // Data retur (result retur)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Ambil data retur penjualan (semua) tanpa filter id_toko
        const laporanData = await getReturDetailNotaPenjualanAll(token);
        console.log("Data Retur Penjualan:", laporanData);
        const dataRows = laporanData[0] ? Object.values(laporanData[0]) : [];
        // Tampilkan semua data tanpa filter id_toko
        setData(dataRows);
        setFilteredData(dataRows);

        // Ambil data "result retur" (semua) tanpa filter id_toko
        const resultReturData = await getResultReturDetailNotaPenjualanAll("ALL", token);
        // console.log("Data Result Retur:", resultReturData);
        const resultRows = resultReturData[0] ? Object.values(resultReturData[0]) : [];
        // Tampilkan semua data result retur tanpa filter id_toko
  ...
  // Handle Filter (hanya berdasarkan kode_retur dan no_pemesanan serta tanggal dan search)
  useEffect(() => {
    let filtered = data;
    if (kodeReturFilter) {
  ...
  const detailNotaJual = (idRetur) => {
    ...
    navigate(`/dashboard/master/menu/notapenjualan/returpenjualan/proses/${idRetur}`);
  };
      ...
      <div className="bg-white relative rounded-md shadow-md border border-gray-200 p-1.5">
        <div className="flex flex-col md:flex-row items-center justify-between mb-2 space-y-4 md:space-y-0">
          ...
          <div className="flex justify-end">
            <button
              onClick={() =>
                navigate("/dashboard/master/menu/notapenjualan/returpenjualan/add")
              }
              className="w-14 h-7 text-xs rounded-md border-2 text-gray-100 bg-blue-900 hover:bg-blue-600 transition duration-300"
            >
              Tambah
            </button>
          </div>
