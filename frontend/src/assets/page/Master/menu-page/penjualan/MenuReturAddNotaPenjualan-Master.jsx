// MenuReturByDetailNotaPenjualan-master.jsx
...
export default function MenuReturNotaDetailPenjualanBesttroli() {
  ...
  useEffect(() => {
    const fetchNoPemesananOptions = async () => {
      try {
        ...
        // Tidak lagi memfilter berdasarkan id_toko, gunakan semua data
        const allData = response[0] ? Object.values(response[0]) : [];
        setNoPemesananOptions(allData);

        // Buat array string dari field no_pemesanan untuk FilterSelect
        const list = allData.map((opt) => opt.no_pemesanan);
        // Hilangkan duplikasi
  ...
  }, [token]); // id_toko tidak lagi dipakai dalam dependency
  ...
  const handleConfirmRetur = async () => {
    ...
    try {
      ...
      const itemsPayload = tableRows.map((row) => ({
        id_header_pemesanan: row.id_header_pemesanan,
      ...
      if (response.success) {
        ...
        setTimeout(() => {
          navigate("/dashboard/master/menu/notapenjualan/returpenjualan");
        }, 2000);
      } else {
        setAlert({
          ...
          <table className="w-full text-xs text-left text-gray-500 border-collapse">
            <thead className="text-xs text-blue-900 uppercase bg-gray-200 sticky top-0">
              <tr>
                ...
                <th className="px-1 py-0.5 border border-gray-500">Qty Retur</th>
                <th className="px-1 py-0.5 border border-gray-500">Total Harga</th>
