...
export default function MenuPelunasanPenjualan() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Tampilkan semua data tanpa filter berdasarkan id_toko
        setData(dataArray);
  ...
  const handleExport = () => {
    ...
    const exportData = sortedData.map((item, index) => ({
      ...
      "Kode Pelunasan": item.kode_pelunasan || "-",
      "ID Akun": item.id_akun || "Data tidak ditemukan",
      "Nama Akun": item.nama_akun || "Data tidak ditemukan",
      "Kode Akuntansi": item.kode_akun_akuntansi || "Data tidak ditemukan",
      "Nama Akuntansi": item.nama_akun_akuntansi || "Data tidak ditemukan",
      "No Pemesanan": item.no_pemesanan || "Data tidak ditemukan",
      "Nama Toko": item.nama_toko || "Data tidak ditemukan",
      "Terbayar": item.terbayar !== undefined
        ? formatRupiah(item.terbayar)
        : "Data tidak ditemukan",
      "Pembayaran": item.total !== undefined
        ? formatRupiah(item.total)
        : "Data tidak ditemukan",
      "Sisa": item.sisa !== undefined
      ...
      "Update At": item.updateAt
        ...
        : "Data tidak ditemukan",
      // Sertakan status_kode untuk pengecekan style
      status_kode: item.status_kode,
    }));

    const worksheet = XLSX.utils.json_to_sheet(exportData);

    // Terapkan style background biru (hex: 4299E1) untuk baris dengan status_kode = 2
    // Perhatikan: Baris header berada di baris 1, data mulai dari baris 2
    const range = XLSX.utils.decode_range(worksheet["!ref"]);
    exportData.forEach((item, idx) => {
      if (item.status_kode === 2) {
        const rowNum = idx + 2; // Data mulai baris ke-2
        for (let C = range.s.c; C <= range.e.c; C++) {
          const cellAddress = XLSX.utils.encode_cell({ r: rowNum - 1, c: C });
          if (worksheet[cellAddress]) {
            worksheet[cellAddress].s = {
              fill: { fgColor: { rgb: "4299E1" } },
            };
          }
        }
      }
            ...
            <thead className="text-xs font-semibold text-blue-900 uppercase bg-gray-200">
              <tr>
                ...
                <th
                  ...
                  onClick={() => handleSort("terbayar")}
                >
                  Terbayar{" "}
                  {sortConfig.key === "terbayar" &&
                    (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                <th
                  scope="col"
              ...
              {currentItems.length > 0 ? (
                currentItems.map((item, index) => {
                  return (
                    <tr
                      key={index}
                      className={`${getRowClass(item)} border-b text-gray-900`}
                    >
                ...
                <tr>
                  <td colSpan={13} className="py-4 px-4 text-center text-gray-500 border-b">
