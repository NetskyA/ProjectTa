...
import { getLaporanMasterToko } from "../../../../../services/apiService";
...
export default function MenuLaporanMasterToko() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log("Data Laporan Master Toko (original):", result);
        ...
        const filteredResult = formattedData.filter(item => parseInt(item.id_toko, 10) !== 1);
  
        setData(filteredResult);
        setFilteredData(filteredResult);
      } catch (err) {
        console.error("Error fetching master toko data:", err);
        setError(err.message || "Failed to fetch master toko data.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const uniqueNamaToko = [...new Set(data.map((item) => item.nama_toko || ""))];
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter((item) => {
        const { id_toko, kode_toko, nama_toko, id_depo, nama_depo, alamat_toko } = item;
    ...
    setCurrentPage(1);
  }, [searchTerm, kodeTokoFilter, namaTokoFilter, namaDepoFilter, data]);
  ...
  const handleExport = () => {
    ...
    const filename = `Master-Toko-${formattedDate}.xlsx`;
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "MasterToko");
    ...
    <div className="py-14" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="head flex justify-between items-center mb-0.5">
        <div className="cover flex items-center">
          ...
          <span className="text-xs font-semibold text-gray-400">Master Toko</span>
        </div>
          ...
          <table className="w-full text-xs text-left text-gray-500 border-collapse border">
            <thead className="text-xs font-semibold text-blue-900 uppercase bg-gray-200 w-full">
              <tr>
                ...
                <th
                  onClick={() => handleSort("alamat_toko")}
                  className="px-2 py-1 w-44 sticky top-0 border border-gray-300 bg-gray-200 z-10 cursor-pointer"
                >
                  Alamat
              ...
              {currentItems.length > 0 ? (
                currentItems.map((item, index) => (
                  <tr key={index} className="bg-white border-b hover:bg-gray-50">
                    ...
                    {/* <td className="px-2 py-1 border">{item.id_toko ?? ""}</td> */}
                    <td className="px-2 py-1 border">{item.kode_toko ?? ""}</td>
                    <td className="px-2 py-1 border">{item.nama_toko ?? ""}</td>
                    <td className="px-2 py-1 border">{item.id_depo ?? ""}</td>
                    <td className="px-2 py-1 border">{item.nama_depo ?? ""}</td>
                    <td className="px-2 py-1 border">{item.alamat_toko ?? ""}</td>
                  </tr>
                ))
              ) : (
                <tr>
