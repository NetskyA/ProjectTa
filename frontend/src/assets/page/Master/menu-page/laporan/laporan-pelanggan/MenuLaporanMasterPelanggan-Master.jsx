  ...
  useEffect(() => {
    ...
    if (kodeFilter)
      filtered = filtered.filter(
        (item) => item.kode_pelanggan_external === kodeFilter
      );
    if (namaFilter)
      filtered = filtered.filter(
        (item) => item.nama_pelanggan_external === namaFilter
      );
    if (statusFilter !== "Semua") {
      filtered = filtered.filter(
        (item) => item.status === (statusFilter === "Aktif" ? 0 : 1)
      );
    }
    if (searchTerm) {
      const term = searchTerm.toLowerCase();
      filtered = filtered.filter((item) =>
        `${item.kode_pelanggan_external} ${item.nama_pelanggan_external} ${item.alamat} ${item.nama_user}`
          .toLowerCase()
          .includes(term)
      );
    }
    ...
    setCurrentPage(1);
  }, [searchTerm, kodeFilter, namaFilter, statusFilter, data]);
...
const handleDeleteClick = (id_master_pelanggan_external) => {
  setDeleteId(id_master_pelanggan_external);
  setIsDialogOpen(true);
};

const confirmDelete = async () => {
  try {
    await deleteMasterPelangganExternal(deleteId, token); // ✅ perbaikan di sini
    setAlert({
...
setTimeout(() => {
    ...
    const updatedData = data.filter(
      (item) => item.id_master_pelanggan_external !== deleteId
    );
    setData(updatedData);
    setFilteredData(updatedData);
  } catch (err) {
    setAlert({
      message: err.message || "Gagal menonaktifkan pelanggan.",
      type: "error",
      visible: true,
    });
  } finally {
...
const handleExport = () => {
  ...
  const exportData = sortedData.map((item, index) => ({
    ...
    Kode_Pelanggan: item.kode_pelanggan_external || "",
    Nama_Pelanggan: item.nama_pelanggan_external || "",
    Alamat: item.alamat || "",
    Created_By: item.nama_user || "",
    ...
    <div className="py-14" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="bg-white flex flex-col md:flex-row flex-wrap rounded-md shadow-md p-4 justify-between items-center border border-gray-200 mb-2">
        <div className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 w-full">
          <FilterSelect
            label="Filter Kode Pelanggan"
            options={[
              ...new Set(
                data.map((item) => item.kode_pelanggan_external || "")
              ),
            ]}
            value={kodeFilter}
          ...
          <FilterSelect
            label="Filter Nama Pelanggan"
            options={[
              ...new Set(
                data.map((item) => item.nama_pelanggan_external || "")
              ),
            ]}
            value={namaFilter}
      ...
      <div className="bg-white rounded-md shadow-md border border-gray-200 p-1.5">
          ...
          <table className="w-full text-xs text-left text-gray-500 border-collapse border">
            <thead className="text-xs font-semibold text-blue-900 uppercase bg-gray-200 w-full">
              <tr>
                ...
                >
                  Nama Pelanggan{" "}
                  {sortConfig.key === "kode_role" &&
                    (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                <th
                  onClick={() => handleSort("kode_role")}
                  className="px-1 py-0.5 w-52 sticky top-0 border border-gray-700 bg-gray-200 text-blue-900 z-10 cursor-pointer"
                >
                  Alamat{" "}
                  {sortConfig.key === "kode_role" &&
                    (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                ...
                <th
                  ...
                  className="px-1 py-0.5 w-14 sticky top-0 border border-gray-700 bg-gray-200 text-blue-900 z-10 cursor-pointer"
                >
                  Status{" "}
                ...
                currentItems.map((item, index) => {
                  ...
                  return (
                    <tr
                      ...
                      className={`text-gray-900 hover:opacity-75 cursor-pointer ${rowBg}`}
                    >
                      <td className="px-1 py-0.5 border border-gray-700">
                        {indexOfFirstItem + index + 1}
                      </td>
                      <td className="px-1 py-0.5 border border-gray-700">
                        {item.kode_pelanggan_external}
                      </td>
                      <td className="px-1 py-0.5 border border-gray-700">
                        {item.nama_pelanggan_external}
                      </td>
                      <td className="px-1 py-0.5 border border-gray-700">
                        {item.alamat}
                      </td>
                      ...
                      {/* <td className="px-1 py-0.5 border border-gray-700">
                        <div className="flex justify-center space-x-2">
                          <button
                            onClick={() =>
                              handleEdit(item.id_master_pelanggan_external)
                            }
                            title="Edit"
                          >
                            <svg
                          ...
                          <button
                            onClick={() =>
                              handleDeleteClick(
                                item.id_master_pelanggan_external
                              )
                            }
                            title="Delete"
                          >
        ...
        <DialogTrueFalse
          ...
          message="Apakah Anda yakin ingin menonaktifkan pelanggan ini?"
