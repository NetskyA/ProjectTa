...
function FilterSelect({ label, options, value, onChange, disabled  }) {
  ...
  const handleSelect = (option) => {
    ...
    onChange(option);                // kirim ke parent
    setShowOptions(false);
  };
  ...
  const handleInput = (e) => {
    ...
    // kosong → anggap user ingin menghapus pilihan
  ...
  return (
    <div ref={wrapperRef} className="relative w-full">
      {/* label */}
      <label className="block text-blue-900 font-semibold text-xs mb-0.5">
        {label}
      </label>
      ...
      <input
        ...
        className={`border border-gray-300 text-xs ${
          disabled ? "bg-gray-100 cursor-not-allowed text-gray-800" : "bg-white"
        } rounded p-1 w-full pr-6`}
      />
...
export default function MenuInsertMasterDataBarangMaster() {
  ...
  const [disableHarga, setDisableHarga] = useState(false);
...
const isUsedInSales = dataArray.some(
  (item) =>
    String(item.id_produk) === String(matchedBarang.id_produk) ||
    String(item.kode_produk) === String(matchedBarang.kode_produk)
      ...
      setFormUpdate({
        id_kategori_adonan_produk: matchedBarang.id_kategori_adonan_produk,
        id_kategori_filling_pertama: matchedBarang.id_kategori_filling_pertama,
        id_kategori_filling_kedua: matchedBarang.id_kategori_filling_kedua,
        id_kategori_topping_pertama: matchedBarang.id_kategori_topping_pertama,
        id_kategori_topping_kedua: matchedBarang.id_kategori_topping_kedua,
        pajak: matchedBarang.pajak,
        margin_kotor: matchedBarang.margin_kotor,
      });
      ...
      // Ambil master kategori adonan
      const adonanMasterList = await getMasterAdonan();
      const adonanList = Array.isArray(adonanMasterList)
        ? adonanMasterList
        : Object.values(adonanMasterList);
      setMasterAdonanKategori(adonanList);
  ...
  const [formUpdate, setFormUpdate] = useState({
    id_kategori_adonan_produk: null,
    id_kategori_filling_pertama: null,
    id_kategori_filling_kedua: null,
    id_kategori_topping_pertama: null,
    id_kategori_topping_kedua: null,
    pajak: 0,
    margin_kotor: 0,
  });
  ...
  const handleExportExcelAdonan = () => {
    if (currentItemsAdonan.length === 0) {
      setAlert({
        message:
          "Tidak ada data adonan yang sedang ditampilkan untuk diekspor.",
        type: "error",
        visible: true,
      });
      return;
    ...
    setAlert({
      message: "Export adonan berhasil!",
      type: "success",
      visible: true,
    });
  ...
  const handleExportExcelBahanBaku = () => {
    if (currentItemsBahan.length === 0) {
      setAlert({
        message:
          "Tidak ada data bahan baku yang sedang ditampilkan untuk diekspor.",
        type: "error",
        visible: true,
      });
      return;
    ...
    setAlert({
      message: "Export bahan baku berhasil!",
      type: "success",
      visible: true,
    });
    setTimeout(() => setAlert({ message: "", type: "", visible: false }), 2000);
  ...
  const handleSubmitUpdateBarang = async () => {
    try {
      setLoading(true);
      setAlert({ message: "", type: "", visible: false });

      // Ambil id kategori adonan dari adonanRows
const getKategoriId = (jenis) => {
  // Cari baris dgn jenis yg sama & kategori sudah terisi
  const withKategori = adonanRows.find(
    (row) =>
      row.jenis_adonan?.toUpperCase() === jenis &&
      row.id_kategori_adonan_produk &&
      row.id_kategori_adonan_produk !== 0
  );

  // Kalau tidak ada satupun yg terisi, baru kirim 0
  return withKategori ? withKategori.id_kategori_adonan_produk : 0;
};

// Ambil semua ID kategori dari formUpdate
const selectedKategoriIds = [
  formUpdate.id_kategori_adonan_produk,
  formUpdate.id_kategori_filling_pertama,
  formUpdate.id_kategori_filling_kedua,
  formUpdate.id_kategori_topping_pertama,
  formUpdate.id_kategori_topping_kedua,
].filter((id) => id && id !== 0); // hanya yang terisi

// ✅ Validasi 1: minimal satu diisi
if (selectedKategoriIds.length === 0) {
  setAlert({
    message: "Minimal satu kategori adonan harus dipilih.",
    type: "warning",
    visible: true,
  });
  setLoading(false);
...
// ✅ Validasi 2: tidak boleh ada ID duplikat
const uniqueIds = new Set(selectedKategoriIds);
if (uniqueIds.size !== selectedKategoriIds.length) {
  setAlert({
    message: "Tidak boleh ada kategori adonan yang sama di antara jenis adonan.",
    type: "error",
    visible: true,
  });
  setLoading(false);
...
const payload = {
  ...
  id_kategori_adonan_produk   : formUpdate.id_kategori_adonan_produk   ?? 0,
  id_kategori_filling_pertama : formUpdate.id_kategori_filling_pertama ?? 0,
  id_kategori_filling_kedua   : formUpdate.id_kategori_filling_kedua   ?? 0,
  id_kategori_topping_pertama : formUpdate.id_kategori_topping_pertama ?? 0,
  id_kategori_topping_kedua   : formUpdate.id_kategori_topping_kedua   ?? 0,
  pajak        : formUpdate.pajak        ?? 0,
  margin_kotor : formUpdate.margin_kotor ?? 0,
};


      console.log("Payload yang dikirim:", payload); // Debug

      const res = await updateMasterDataBarang(payload);

      if (res.success) {
        setAlert({
          message: res.message || "Berhasil memperbarui data produk.",
          type: "success",
          visible: true,
        });
        setTimeout(() => {
        ...
        throw new Error(res.message || "Gagal memperbarui data produk.");
      }
    } catch (err) {
    ...
    <div className="py-14" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="head flex justify-between items-center">
        <div className="flex items-center">
          ...
          <Link
            to="/dashboard/master/menu/laporan/databarang"
            className="text-xs font-semibold text-blue-900"
          >
            Laporan Produk
          </Link>
      ...
      <div className="bg-white flex rounded-md shadow-md p-3 justify-between items-center border border-gray-200 mb-1">
        ...
        <Link to="/dashboard/master/menu/laporan/databarang">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
            <div>
              ...
              <input
                ...
                readOnly
                disabled
                className="mt-1 w-full border text-sm border-gray-300 rounded p-1 bg-gray-200 cursor-not-allowed "
              />
            </div>
            <div>
              <label className="block font-medium text-blue-900">
...
<input
  ...
  min="0"
  max="100"
  value={(formUpdate.margin_kotor * 100).toFixed(2)}
  onChange={(e) =>
    setFormUpdate((prev) => ({
      ...prev,
      margin_kotor: parseFloat(e.target.value) / 100,
    }))
  }
  className="mt-1 w-full border text-sm cursor-not-allowed border-gray-300 rounded p-1 text-right"
  disabled
/>
  ...
  <input
    ...
    min="0"
    max="100"
    value={(formUpdate.pajak * 100).toFixed(2)}
    onChange={(e) =>
      setFormUpdate((prev) => ({
        ...prev,
...
<fieldset className="border border-blue-400 rounded-md p-2 w-52 shadow-sm">
  <legend className="text-sm font-bold text-blue-900">Catatan</legend>
  <div className="flex-row text-xs">
    {/* <div className="flex">
      <button
        onClick={handleSubmitUpdateBarang}
        type="submit"
        className="flex items-center justify-center gap-2 bg-blue-800 text-white text-sm w-full h-8 rounded-md hover:bg-blue-950 transition duration-300"
      >
        <img src={LogoSave} className="w-6 h-6" alt="Simpan" />
  ...
  {/* Catatan */}
  <div className="border border-gray-400 rounded-md h-full p-1 flex flex-col justify-start bg-gray-50">
    <p className="text-xs italic text-gray-700  overflow-hidden">
      {disableHarga ? (
        <span className="text-red-500">
          Produk sudah pernah digunakan di penjualan. Laba dan Pajak tidak dapat diubah.
        </span>
      ) : (
        "-"
      )}
          ...
          <div className="w-2/6">
            <div className="relative">
              ...
              <input
                type="text"
                id="simple-search"
                className="bg-gray-200 cursor-not-allowed  border border-gray-300 text-gray-900 text-xs h-8 rounded-md focus:ring-primary-500 focus:border-primary-500 block w-full pl-10 p-2"
                placeholder="Search"
                onChange={(e) => {
            ...
            <tbody>
              {currentItemsAdonan.length === 0 ? (
                ...
                currentItemsAdonan.map((row, i) => {
                  // Dapatkan kategori yang cocok
                  const selectedKategori = masterAdonanKategori.find(
                    (k) =>
                      String(k.id_kategori_adonan_produk) ===
                      String(row.id_kategori_adonan_produk)
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-2 py-1 border border-gray-500 cursor-not-allowed text-black uppercase">
                        <FilterSelect
                         disabled={true} 
...
onChange={(selected) => {
  // selected === null  artinya pilihan dihapus
  const newKategoriId    = selected ? selected.value  : 0;
  const newKategoriLabel = selected ? selected.label : "";

  /* ── perbarui semua baris dengan jenis_adonan yg sama ── */
  setAdonanRows((prev) =>
    prev.map((r) =>
      r.jenis_adonan?.toUpperCase() === row.jenis_adonan.toUpperCase()
        ? {
            ...r,
            id_kategori_adonan_produk: newKategoriId,
            nama_kategori: newKategoriLabel,
          }
        : r
    )
  );

  /* ── sinkronkan ke formUpdate supaya payload ikut berubah ── */
  const jenis = row.jenis_adonan.toUpperCase();
  setFormUpdate((prev) => ({
    ...prev,
    ...(jenis === "ADONAN"          && { id_kategori_adonan_produk   : newKategoriId }),
    ...(jenis === "FILLING PERTAMA" && { id_kategori_filling_pertama : newKategoriId }),
    ...(jenis === "FILLING KEDUA"   && { id_kategori_filling_kedua   : newKategoriId }),
    ...(jenis === "TOPPING PERTAMA" && { id_kategori_topping_pertama : newKategoriId }),
    ...(jenis === "TOPPING KEDUA"   && { id_kategori_topping_kedua   : newKategoriId }),
  }));
}}
      ...
      <div className="bg-white mt-2 rounded-md border border-gray-200 shadow-md p-1.5 w-full">
        ...
        {/* TABLE BAHAN BAKU */}
        <div className="overflow-x-auto w-full" style={{ height: "40vh" }}>
          <table
