...
import { getLaporanMasterRole } from "../../../../../services/apiService";
...
export default function MenuLaporanMasterRole() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log("Data Laporan Master Role (original):", result);
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter((item) => {
        const { id_role, kode_role, nama_role } = item;
    ...
    setCurrentPage(1);
  }, [searchTerm, namaRoleFilter, data]);
  ...
  const handleExport = () => {
    ...
    const filename = `Master-Role-${formattedDate}.xlsx`;
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "MasterRole");
    ...
    <div className="py-14" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="head flex justify-between items-center mb-0.5">
        <div className="cover flex items-center">
          ...
          <span className="text-xs font-semibold text-gray-400">Master Role</span>
        </div>
      ...
      <div className="bg-white flex flex-wrap rounded-md shadow-md p-1.5 justify-start items-center border border-gray-200 mb-2 gap-4">
        <div className="flex flex-col text-xs">
          ...
          <select
            ...
            onChange={(e) => setNamaRoleFilter(e.target.value)}
          >
            <option value="">Semua</option>
            {uniqueNamaRole.map((nama, index) => (
        ...
        <div className="overflow-x-auto w-full" style={{ height: "56vh" }}>
          <table className="w-full text-xs text-left text-gray-500 border-collapse border">
            <thead className="text-xs font-semibold text-blue-900 uppercase bg-gray-200 w-full">
              <tr>
                <th className="px-2 py-1 w-8 sticky top-0 border border-gray-700 bg-gray-200 z-10">No</th>
                {/* <th
...
>
                    ...
                    {/* <td className="px-1 py-0.5 border border-gray-700">{item.id_role ?? ""}</td> */}
