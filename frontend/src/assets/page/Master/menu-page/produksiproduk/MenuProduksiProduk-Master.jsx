...
import { getMasterProduksiProduk } from "../../../../services/apiService";
...
export default function MenuNotaPembelianBasetroli() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const res = await getMasterProduksiProduk(token);
        let rows = [];
  ...
  const detailGabunganPermintaan = (id) => {
    // console.log("Navigating to detail with ID:", id);
    navigate(`/dashboard/master/menu/produksiproduk/detail/${id}`);
  };
      ...
      <div className="bg-white rounded-md shadow-md border border-gray-200 p-1.5 mb-2">
        <div className="flex items-center justify-center mb-2 relative">
          ...
          <div className="w-fit flex absolute right-1 items-center justify-center bg-gray-200 rounded-md p-0.5 cursor-pointer">
            <Link to="/dashboard/master/menu/produksiproduk/add">
                ...
                pageRows.map((r, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Kitchen */}
                      <td className="px-2 py-0.5 border border-gray-500 uppercase">
                      {r.tanggal_verifikasi_pr
                          ? r.tanggal_verifikasi_pr.slice(0,10)
                          : "-"}
                      </td>
                      ...
                      {r.status_produksi === 1
                          ? "PRODUKSI SELESAI"
                          : "MENUNGGU VERIFIKASI PRODUKSI"}
                      </td>

                      {/* Status Gabungan */}
                      <td className="px-2 py-0.5 border border-gray-500">
                      {r.status_bukti_pengeluran === 0
                          ? "MENUNGGU VERIFIKASI B. PENGELUARAN"
