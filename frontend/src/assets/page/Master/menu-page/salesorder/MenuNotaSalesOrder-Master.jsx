// MENUMASTERSALESORDER.jsx

import { useState, useEffect, useMemo, useRef } from "react";
import { Link, useNavigate } from "react-router-dom";
import { getMasterSalesOrderKitchen } from "../../../../services/apiService";
// import { onSoUpdate, offSoUpdate } from "../../../../services/socketService";
      ...
      {showOptions && filteredOptions.length > 0 && (
        <ul className="absolute z-50 w-full bg-white border border-gray-300 rounded-md max-h-40 overflow-y-auto mt-1">
          {filteredOptions.map((opt, i) => (
            <li
              key={i}
              className="px-1 py-1.5 hover:bg-gray-200 cursor-pointer text-sm"
              onClick={() => {
                setInputValue(opt);
                onChange(opt);
                setShowOptions(false);
...
export default function MenuNotaPembelianBasetroli() {
  ...
  // useEffect(() => {
  //   function handleSoUpdate(payload) {
  //     setData(payload);
  //   }
  //   onSoUpdate(handleSoUpdate);
  //   return () => offSoUpdate(handleSoUpdate);
  // }, []);
  // filters
...
useEffect(() => {
  (async () => {
    try {
      ...
      // ➊ Filter keluar baris yang status_batal=1 & status_batal_text="Batal"
      const visibleRows = rows.filter(
        (r) => !(r.status_batal === 1 && r.status_batal_text === "Batal")
      );
  ...
  useEffect(() => {
    let f = data;
    if (kodePesanan)
      f = f.filter((r) => r.kode_pesanan_pembelian === kodePesanan);
    if (kodeSalesOrder)
      f = f.filter((r) => r.kode_sales_order === kodeSalesOrder);
    if (namaKitchen) f = f.filter((r) => r.nama_kitchen === namaKitchen);
    if (namaStore) f = f.filter((r) => r.nama_store === namaStore);
    if (tanggalTransaksiFilter)
      f = f.filter((r) => r.tanggal_transaksi === tanggalTransaksiFilter);
    if (tanggalKirimFilter)
      f = f.filter((r) => r.tanggal_kirim === tanggalKirimFilter);
    if (searchTerm) {
  ...
  const uniqueKodePesanan = [
    ...new Set(data.map((r) => r.kode_pesanan_pembelian)),
  ];
  ...
  const detailNotaPembelian = (id) => {
    // console.log("Navigating to detail with ID:", id);
    navigate(`/dashboard/master/menu/salesorder/detail/${id}`);
  };
    ...
    <div className="py-14" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="bg-white flex flex-col md:flex-row rounded-md shadow-md p-2 justify-between items-center border border-gray-200 mb-1">
        <div className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 w-full">
          <FilterSelect
            label="No. Pemesanan"
            options={uniqueKodePesanan}
            value={kodePesanan}
            onChange={setKodePesanan}
          />
          <FilterSelect
            label="No. SO"
            options={uniqueSalesOrder}
            value={kodeSalesOrder}
            onChange={setKodeSalesOrder}
          />
          <FilterSelect
            label="Kitchen"
            options={uniqueKitchenNames}
            value={namaKitchen}
            onChange={setNamaKitchen}
          />
          <FilterSelect
            label="Store"
            options={uniqueStoreNames}
            value={namaStore}
            onChange={setNamaStore}
          />
          <div className="flex flex-col text-xs">
            ...
            <input
              ...
              value={tanggalTransaksiFilter}
              onChange={(e) => setTanggalTransaksiFilter(e.target.value)}
            />
          </div>
          <div className="flex flex-col text-xs">
            <label className="mb-1 text-blue-900 font-semibold">
            ...
            <input
              type="date"
              className="border border-gray-300 w-44 h-7 px-1 rounded-md text-xs"
              value={tanggalKirimFilter}
              onChange={(e) => setTanggalKirimFilter(e.target.value)}
            />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-md shadow-md border border-gray-200 p-1.5 mb-2">
        <div className="flex items-center justify-center mb-2 relative">
          ...
          {/* <div className="w-fit flex absolute right-1 items-center justify-center bg-gray-200 rounded-md p-0.5 cursor-pointer">
            <Link to="/dashboard/adminpembelian/menu/pembelianbarang">
          ...
          <table className="min-w-max whitespace-nowrap text-xs text-left text-gray-500 border-collapse border">
            <thead className="text-xs font-semibold text-blue-900 uppercase bg-gray-200">
              <tr>
                ...
                <th
                  scope="col"
                  className="px-2 py-0.5 w-52 cursor-pointer sticky top-0 border border-gray-500 bg-gray-200 z-10"
                  onClick={() => handleSort("nama_kitchen")}
                >
                  Kitchen
                  {sortConfig.key === "nama_kitchen" &&
                    (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                <th
                  scope="col"
                ...
                <th
                  scope="col"
                  className="px-2 py-0.5 w-52 cursor-pointer sticky top-0 border border-gray-500 bg-gray-200 z-10"
                  onClick={() => handleSort("nama_store")}
                >
                  Store
                  {sortConfig.key === "nama_store" &&
                    (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                ...
                <th
                  ...
                  onClick={() => handleSort("nama_pesanan")}
                >
                  Jenis Pembelian
                  {sortConfig.key === "nama_pesanan" &&
                    (sortConfig.direction === "asc" ? " ▲" : " ▼")}
                </th>
                <th
                  scope="col"
                ...
                <th
                  scope="col"
                  className="px-2 py-0.5 max-w-lg sticky top-0 border border-gray-500 bg-gray-200 z-10"
                >
                  Catatan
                </th>
              <th
                ...
                pageRows.map((r, i) => {
                  ...
                  return (
                    ...
                    key={r.id_master_pesanan_pembelian}
                      onDoubleClick={() =>
                        detailNotaPembelian(r.id_master_pesanan_pembelian)
                      }
                      className={`border-b text-black cursor-pointer hover:opacity-80 hover:text-black ${rowBgClass}`}
                    >
