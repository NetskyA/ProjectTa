...
import { getLaporanPenjualanNettroli } from "../../../services/apiService";
...
// Assume 'namaToko' is defined elsewhere in your code

// Construct filename with random number
...
console.log(filename);
    ...
    const exportData = sortedData.map((item, index) => ({
      ...
      Tanggal: item.createAt ? formatDate(item.createAt) : "Data tidak tersedia",
    ...
    <div className="py-14" style={{ fontFamily: "sans-serif" }}>
      ...
      <div className="bg-white rounded-md shadow-md border border-gray-200 p-1.5 w-full">
        <div className="flex flex-col md:flex-row items-center justify-between mb-2 space-y-4 md:space-y-0">
          ...
          <div className="w-fit flex items-center justify-center bg-gray-200 rounded-md p-0.5 cursor-pointer" onClick={handleExport}>
            <button
               // Attach the export handler here
          ...
          <table className="w-full text-xs text-left text-gray-500 border-collapse border">
            <thead className="text-xs font-semibold text-blue-900 uppercase bg-gray-200 w-full">
              <tr>
                <th
                  scope="col"
                  className="px-2 py-3 w-10 cursor-pointer sticky top-0 border border-gray-300 bg-gray-200 z-10"
            ...
            <tbody>
              {currentItems.length > 0 ? (
                ...
                <tr>
                  <td
                    colSpan={17} // Sesuaikan dengan jumlah kolom
